...
var detailsPage = fs.readFileSync('templates/detailPage.html');
...
function imageNamesToTags(fileNames) {
  return fileNames.map(function(fileName) {
    return `<a href="detailPage.${fileName}"><img src="${fileName}" alt="${fileName}"></a>`;
  });
}

function singleNameToTag(clickedFileName) {
    return '<img src="'+clickedFileName+'" alt="'+clickedFileName + '">';
    //'<img src="' + clickedFileName + '" alt="' + clickedFileName +
}
...
function buildDetailPage(imageTag){
  var location = 'data/' + imageTag.split('.')[0] + '.json';
  console.log(location);

  var imageData = JSON.parse(fs.readFileSync(location));
  return template.render('detailPage.html', {
    imageTags: singleNameToTag(imageTag),
...
function serveDetailsPage(req, res, clicked){


  res.setHeader('Content-Type', 'text/html');
  res.end(buildDetailPage(clicked));
}
...
function createProfile(newFileName, newTitle, newDescription){
  var contents = '{"title":"'+ newTitle + '", "description":"' + newDescription + '"}';
  fs.writeFile('data/' + newFileName.split(".")[0] + ".json", contents, function(err){
    if(err){
      console.error(err);
      res.statusCode = 500;
      res.statusMessage = "Server Error";
...
function uploadImage(req, res) {
  multipart(req, res, function(req, res) {
    ...
    fs.writeFile('images/' + req.body.image.filename, req.body.image.data, function(err){
      ...
      createProfile(req.body.image.filename, title, description);
...
function handleRequest(req, res) {
  ...
  var parts = urlParts.pathname.split('.');
  if(parts[0] && parts[0] == "/detailPage"){
    serveDetailsPage(req, res, parts[1] + "." + parts[2]);
